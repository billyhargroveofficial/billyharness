...
func (s *Server) routeSpecs() []routeSpec {
	return []routeSpec{
		...
		{Method: "GET", Pattern: "/v1/jobs", Handler: s.handleListJobs, Summary: "List durable multi-agent jobs", Response: "JobListResponse"},
		{Method: "POST", Pattern: "/v1/jobs", Handler: s.handleCreateJob, Summary: "Create a bounded durable multi-agent job", Request: "CreateJobRequest", Response: "JobResponse"},
		{Method: "GET", Pattern: "/v1/jobs/{id}", Handler: s.handleGetJob, Summary: "Fetch a durable multi-agent job", Response: "JobResponse"},
		{Method: "GET", Pattern: "/v1/jobs/{id}/attempts", Handler: s.handleListJobAttempts, Summary: "Page durable job attempts", Response: "JobAttemptPage"},
		{Method: "GET", Pattern: "/v1/jobs/{id}/artifacts", Handler: s.handleListJobArtifacts, Summary: "Page durable job artifacts", Response: "JobArtifactPage"},
		{Method: "POST", Pattern: "/v1/jobs/{id}/run", Handler: s.handleRunJob, Summary: "Start or re-admit a durable job", Request: "{}", Response: "JobResponse"},
		{Method: "POST", Pattern: "/v1/jobs/{id}/pause", Handler: s.handlePauseJob, Summary: "Pause a durable job at a safe boundary", Request: "{}", Response: "JobResponse"},
		{Method: "POST", Pattern: "/v1/jobs/{id}/resume", Handler: s.handleResumeJob, Summary: "Resume a paused or waiting durable job", Request: "{}", Response: "JobResponse"},
		{Method: "POST", Pattern: "/v1/jobs/{id}/cancel", Handler: s.handleCancelJob, Summary: "Persist operator cancellation for a durable job", Request: "{}", Response: "JobResponse"},
