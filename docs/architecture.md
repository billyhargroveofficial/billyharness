...
| `internal/gateway` | HTTP adapter for sessions, benchmark artifacts, durable-job admission/control, session run locking, persistence, replay, and inspection. | `agent`, `attachments`, `checkpoint`, `clientux`, `clientux/projector`, `config`, `credentials`, `eventlog`, `gatewayapi`, `jobs`, `jobservice`, `jobstore`, `mcpstatus`, `modelinfo`, `protocol`, `provider`, `runstate`, `runtimehost`, `secrets`, `tools`, `trace` | Must directly import `eventlog` and keep lifecycle semantics there. Durable execution stays behind the narrow `jobservice` controller. Server DTOs and shared client transport helpers belong to `gatewayapi`/`gatewayclient`. |
| `internal/gatewayapi` | Shared gateway HTTP request/response DTOs, including bounded durable-job requests, URL normalization, readiness, scoped auth-header attachment, and unavailable-hint helpers. | `gatewayauth`, `jobs`, `protocol`, `serviceops` | Managed local credentials may be attached only to loopback requests; remote requests require an explicit process credential. Must not import gateway server, clients, runtime, provider, tools, TUI, or Telegram. |
| `internal/gatewayauth` | Secure gateway transport-token resolution, bounded legacy migration, and private atomic local persistence. | `config` | Must stay independent of gateway server/client/UI/runtime packages. Managed-file creation is loopback startup policy owned by the command adapter; this package only resolves or persists when explicitly called. Private managed persistence is Darwin/Linux-only until equivalent owner/mode/link/locking guarantees exist elsewhere. |
...
| `internal/jobagent` | Isolated provider/tool adapter for one bounded durable-job invocation, exact route pinning, provider usage normalization, and strict supervisor output parsing. | `config`, `jobruntime`, `jobs`, `modelinfo`, `protocol`, `provider`, `runtimehost`, `tools` | Must not select stages, IDs, budgets, deadlines, or authority from model output. Ambient profiles, memory, hooks, MCP, helper providers, and unscoped tools remain disabled. |
| `internal/jobstore` | Durable JSONL event storage, deterministic job replay, disposable snapshots, single-owner locking, and content-verified job artifacts. | `jobs` | Must not import provider, agent, tools, gateway, or UI packages. Canonical events must be fsynced before snapshots, completed-record corruption must fail closed, and runtime authority must exclude the store root and its ancestors from worker writes. FileStore runtime support is Darwin/Linux until equivalent owner-only filesystem controls exist elsewhere. |
| `internal/jobruntime` | Provider-neutral durable scheduler: immutable workflow traversal, bounded parallel invocation, barriers, supervisor decisions, cancellation, budgets, and crash recovery. | `jobs`, `jobstore`, `secrets` | Must not import provider, agent, tools, gateway, or UI packages. External work is persisted with explicit dispatch provenance; read work is at-least-once, while ambiguous writer recovery fails closed. |
| `internal/jobservice` | Daemon-lifetime ownership of background job step loops, recovery admission, pause/resume/cancel coordination, and process-local activity observations. | `jobs`, `jobstore`, `secrets` | Must not construct providers, parse HTTP, or own store closure. Shutdown is process control and never persists operator cancellation. |
...
| `internal/provider` | Provider clients, OpenAI-compatible and Codex request building, streaming parsers, auth integration, and provider-backed web summary adapter. | `attachments`, `codexauth`, `config`, `credentials`, `modelinfo`, `protocol`, `secrets`, `webtools` | Must not import tools, gateway, TUI, Telegram, or bench. |
...
| `internal/tui` | Bubble Tea terminal UI, gateway session mode, full-screen durable-job control center, rendering, input handling, and persisted chat blocks. | `attachments`, `clientux`, `clientux/projector`, `clipboard`, `commandregistry`, `config`, `credentials`, `displayfmt`, `filesearch`, `gatewayapi`, `gatewayclient`, `jobs`, `mcpstatus`, `memory`, `modelinfo`, `promptcommands`, `protocol`, `toolrender`, `tui/jobclient`, `tui/render`, `tui/runtimeclient`, `tui/selection`, `tui/transcript` | Must not import gateway server internals, job runtime/store/service, agent, provider, or tools directly. Local chat runtime goes through `tui/runtimeclient`; durable-job control goes through `tui/jobclient` and never creates a TUI-owned scheduler. |
| `internal/tui/jobclient` | Bubble Tea command adapter for typed durable-job gateway create/list/show/control and bounded attempt/artifact pages. | `gatewayapi`, `gatewayclient` | Must remain a client control-plane adapter: no gateway server, `jobs`, job runtime/store/service, provider, tools, TUI model, or rendering imports. The gateway remains the sole durable owner. |
