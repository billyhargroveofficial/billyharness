...
| `internal/gatewayapi` | Shared gateway HTTP request/response DTOs, including bounded durable-job requests, URL normalization, readiness, auth header, and unavailable-hint helpers. | `config`, `jobs`, `protocol`, `serviceops` | Must not import gateway server, clients, runtime, provider, tools, TUI, or Telegram. |
