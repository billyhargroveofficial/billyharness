...
| `internal/agentclub/hhapplicant` | Narrow read-only HH applicant-tool review queue adapter that builds gateway ingress events from fixed local command output. | `gatewayapi`, `ingress` | Must not become a generic project runner, scheduler, HH mutator, browser/auth adapter, raw API caller, raw SQL caller, or gateway server client. |
