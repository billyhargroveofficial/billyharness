...
| `internal/gatewaybase` | Gateway URL normalization, readiness, auth header, and unavailable-hint helpers shared by gateway and clients. | `config` | Must stay transport-helper only; no gateway server, HTTP client wrappers, runtime, provider, TUI, or Telegram imports. |
...
| `internal/toolrender` | Shared tool display labels and argument summaries for clients. | `displayfmt`, `protocol` | Must not import TUI, Telegram, gateway, or tools. |
