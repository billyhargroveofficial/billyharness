...
| `internal/telegrambot` | Telegram adapter, rendering, command handling, and gateway client wrapper. | `clientux`, `clientux/projector`, `config`, `credentials`, `eventlog`, `gatewayapi`, `gatewayclient`, `mcpstatus`, `memory`, `modelinfo`, `protocol`, `toolrender` | Must not import gateway server internals. |
